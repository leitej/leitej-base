 ...
 * @author Julio Leite
 */
public final class CommunicationSecureListener extends
		AbstractCommunicationListener<CommunicationSecureFactory, CommunicationSecureListener, CommunicationSecureHost, CommunicationSecureGuest> {
	 ...
	 */
	CommunicationSecureListener(final CommunicationSecureFactory factory, final int port, final int backlog,
			final InetAddress bindAddr) throws SecurityException, SocketException, IOException {
		super(factory, port, backlog, bindAddr);
	}

	@Override
	protected CommunicationSecureHost getCommunicationSessionHost(final CommunicationSecureFactory factory,
			final Socket socket) throws ConnectionLtException {
		try {
			return new CommunicationSecureHost(factory, socket);
