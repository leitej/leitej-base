 ...
 * @author Julio Leite
 */
abstract class AbstractSimpleKeyStore extends AbstractKeyStore {
	 ...
	 */
	protected AbstractSimpleKeyStore(final InputStream is, final Password password)
			throws KeyStoreLtException, IOException {
		super(KeyStoreEnum.UBER, is, password);
					...
					while (aliases.hasMoreElements()) {
						...
						if (isKeyEntry(alias)) {
							if (getPrivateKey(alias) != null) {
								super.setPrivateKeyEntry(alias, getPrivateKey(alias), newPassword, getKeyCertificateChain(alias));
