 ...
 * @author Julio Leite
 */
public final class BigBinaryData implements RandomAccessBinaryItf, Serializable {

	private static final long serialVersionUID = 4282770784264419884L;

	private static final Logger LOG = Logger.getInstance();

	private static final CacheWeak<String, BigBinaryData> INSTANCE_MAP = new CacheWeak<>();

	public static final BigBinaryData valueOf(final String baseDirectory, final long id) throws IOException {
	...
	private static final boolean delete(final String absolutePath) {
		...
		synchronized (INSTANCE_MAP) {
			final BigBinaryData bigBinary = INSTANCE_MAP.get(absolutePath);
			try {
				if (bigBinary != null && !bigBinary.released) {
					bigBinary.internalRelease();
				}
	...
	public BigBinaryData() throws IOException {
		this.temporary = true;
	...
	private BigBinaryData(final String absolutePath, final long id) throws IOException {
